import sys
#sys.path.append('/mnt1/xulu_zhang/git_projects/Diffusion/Text2Image/main')
sys.path.append('/home/xulu/Text2Image/main')
import argparse
import hashlib
import itertools
import json
...
def collate_fn(examples, with_prior_preservation):
    ...
    batch = {"input_ids": input_ids, "pixel_values": pixel_values, "reference_prompt_ids": reference_input_ids, "placeholder_index":placeholder_index}
    return batch
...
class CompositionalDreamboothDataset(Dataset):
    """
...
def main(args):
    ...
    def save_model_hook(models, weights, output_dir):
        for model in models:
            if isinstance(model, type(accelerator.unwrap_model(text_encoder.text_model.embeddings))):
                continue
    ...
    optimizer = optimizer_class(
        itertools.chain(text_encoder.get_input_embeddings().parameters(), unet.parameters())
        if args.modifier_token is not None
    ...
    train_dataset = CompositionalDreamboothDataset(
    ...
    if args.modifier_token is not None:
        unet, text_encoder, optimizer, train_dataloader, lr_scheduler, Embeddings = accelerator.prepare(
            unet, text_encoder, optimizer, train_dataloader, lr_scheduler, text_encoder.text_model.embeddings
        )
    else:
        unet, optimizer, train_dataloader, lr_scheduler, Embeddings = accelerator.prepare(
            unet, optimizer, train_dataloader, lr_scheduler, text_encoder.text_model.embeddings
        )
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet), accelerator.accumulate(text_encoder):
                ...
                if args.with_prior_preservation:
                    ...
                    loss = F.mse_loss(model_pred.float(), target.float(), reduction="mean") * args.recons_loss_weight
