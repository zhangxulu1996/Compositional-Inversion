...
"""
使用l2 loss应用到custom difffusion中
"""
...
def collate_fn(examples, with_prior_preservation):
    placeholder_index = [example["placeholder_index"] for example in examples]
    input_ids = [example["instance_prompt_ids"] for example in examples]
    reference_input_ids = [example["reference_prompt_ids"] for example in examples]
    ...
    placeholder_index = torch.stack(placeholder_index).squeeze(1)
    reference_input_ids = torch.cat(reference_input_ids, dim=0)
    pixel_values = pixel_values.to(memory_format=torch.contiguous_format).float()
    mask = mask.to(memory_format=torch.contiguous_format).float()
    batch = {"input_ids": input_ids, "pixel_values": pixel_values, "mask": mask.unsqueeze(1), "reference_prompt_ids": reference_input_ids, "placeholder_index":placeholder_index}
    return batch
...
class CompositionalCustomDiffusionDataset(Dataset):
    """
    ...
    def __init__(
        ...
        for concept in concepts_list:
            inst_img_path = [
                (x, concept["instance_prompt"], concept["expected_class"]) for x in Path(concept["instance_data_dir"]).iterdir() if x.is_file()
            ]
    ...
    def __getitem__(self, index):
        example = {}
        instance_image, instance_prompt, class_noun = self.instance_images_path[index % self.num_instance_images]
        ...
        for i, w in enumerate(instance_prompt.split(' ')):
            if '<' in w:
                example["placeholder_index"] = torch.tensor([i+1])
                unique_identifier = w
                break
        ...
        example["reference_prompt_ids"] = self.tokenizer(
                instance_prompt.replace(unique_identifier, class_noun),
                truncation=True,
                padding="max_length",
                max_length=self.tokenizer.model_max_length,
                return_tensors="pt",
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default='none',
        help="The loss function for embedding, choose from ['none', 'l1', 'l2', 'cosine']."
    )
    parser.add_argument(
        "--anchor_loss_weight",
        type=float,
        default=0.0,
        help="The weight of embedding loss."
    )
    parser.add_argument(
        "--pretrained_model_name_or_path",
        type=str,
...
def main(args):
    ...
    train_dataset = CompositionalCustomDiffusionDataset(
    ...
    if args.modifier_token is not None:
        custom_diffusion_layers, text_encoder, optimizer, train_dataloader, lr_scheduler, Embeddings = accelerator.prepare(
            custom_diffusion_layers, text_encoder, optimizer, train_dataloader, lr_scheduler, text_encoder.text_model.embeddings
        )
    else:
        custom_diffusion_layers, optimizer, train_dataloader, lr_scheduler, Embeddings = accelerator.prepare(
            custom_diffusion_layers, optimizer, train_dataloader, lr_scheduler, text_encoder.text_model.embeddings
        )
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet), accelerator.accumulate(text_encoder):
                ...
                if args.with_prior_preservation:
                    ...
                    mask = torch.chunk(batch["mask"], 2, dim=0)[0]  # 加mask的地方才计算loss
                    # Compute instance loss
                    loss = F.mse_loss(model_pred.float(), target.float(), reduction="none")
                    loss = ((loss * mask).sum([1, 2, 3]) / mask.sum([1, 2, 3])).mean() * args.recons_loss_weight
                ...
                if args.anchor_loss == 'l2':
                    with torch.no_grad():
                        reference_input_embedings = Embeddings(batch["reference_prompt_ids"])  # token_embeddings + position_embeddings
                    optimized_embeddings, _ = torch.chunk(input_embeddings, 2, dim=0)
                    anchor_loss = F.mse_loss(optimized_embeddings, reference_input_embedings.detach(), reduction="mean")
                    loss = loss + args.anchor_loss_weight * anchor_loss
...
# 1 concept
# def get_args(args):
#     tokens = ["<cute-cat>", "<black-dog>", "<blue-chair>", "<brown-table>", "<white-flower>", "<brown-barn>", "<wooden-pot>", "<cat-statue>", "<red-teapot>", "<teddy-bear>"]
#     cls = ["cat", "dog", "chair", 'table', 'flower', 'barn', 'wooden_pot', 'cat_statue', 'teapot', 'teddybear']
#     concepts_info = json.load(open(args.concepts_list))
#     output_dir = args.output_dir
#     for i in range(len(tokens)):
#         args.modifier_token = tokens[i]
#         args.output_dir = output_dir.format(cls[i])
#         if os.path.exists(os.path.join(args.output_dir, 'pytorch_custom_diffusion_weights.bin')):
#             continue
#         args.concepts_list = [concepts_info[i]]
#         return args
